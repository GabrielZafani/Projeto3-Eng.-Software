...
function ranquearProdutos(vendas, limite) {
    const porProduto = vendas.reduce((acumulador, venda) => {
        const nome = venda.produto;
    ...
    const faturamentoGeral = Object.values(porProduto)
...
function somarFaturamento(vendas) {
    return vendas.reduce((total, venda) => total + numeroSeguro(venda.quantidade) * numeroSeguro(venda.valor_unitario), 0);
}
function recortarUltimaSemana(vendas) {
    // .filter() nº 1: fora as linhas sem data utilizável.
    const comData = vendas.filter((venda) => ehDataValida(venda.data_venda));
    if (comData.length === 0) {
        return null;
    }
    ...
    const fim = comData.reduce((maior, venda) => (venda.data_venda > maior ? venda.data_venda : maior), comData[0].data_venda);
    const inicio = somarDias(fim, -(DIAS_DA_JANELA - 1));
    // .filter() nº 2: a janela em si.
    const daJanela = comData.filter((venda) => venda.data_venda >= inicio && venda.data_venda <= fim);
    // .filter() nº 3: a semana imediatamente anterior, só para comparar.
    const fimAnterior = somarDias(inicio, -1);
    const inicioAnterior = somarDias(fimAnterior, -(DIAS_DA_JANELA - 1));
    const daJanelaAnterior = comData.filter((venda) => venda.data_venda >= inicioAnterior && venda.data_venda <= fimAnterior);
    const faturamento = somarFaturamento(daJanela);
    const faturamentoAnterior = somarFaturamento(daJanelaAnterior);
    return {
        inicio,
        fim,
        faturamento,
...
// ====================================================================
function atualizarDestaques(vendas) {
...
function desenharPeriodo(recorte) {
    if (recorte === null) {
    ...
    escreverTexto('periodo-intervalo', formatarDiaMes(recorte.inicio) + ' a ' + formatarDiaMes(recorte.fim));
    const plural = recorte.quantidadeVendas === 1 ? 'venda' : 'vendas';
    const pluralCategoria = recorte.categorias === 1 ? 'categoria' : 'categorias';
    escreverTexto('periodo-detalhe', recorte.quantidadeVendas + ' ' + plural + ' · ' + recorte.unidades + ' un. · '
    ...
    escreverTexto('periodo-variacao', sinal + recorte.variacao.toFixed(1) + '% em relação à semana anterior');
}
...
function prepararLinhas(vendas) {
